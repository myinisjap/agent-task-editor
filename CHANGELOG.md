...
### Added
- **Per-repo runtime containers (`runtime_image`)** — a repo can now set `runtime_image` to a Docker image ref, and agent CLI runs for that repo execute inside a long-lived container from that image (`docker exec`) instead of in the backend container. Lets a repo bring its own language toolchain without growing `backend/Dockerfile`. Empty `runtime_image` (the default) is unchanged: everything still runs in-process in the backend container. The repo, `/tmp`, the MCP sidecar binary, and provider credential directories (`~/.claude`, `~/.codex`, `~/.qwen`) are bind-mounted at the same paths into the container; see `docs/runtime-containers.md` for the mount contract, the `HOME=/home/vscode` devcontainer-image assumption, and the Docker socket requirement. If the container can't be started, the task escalates to `waiting_human` instead of silently falling back to running in-process. The `mcp-server` sidecar binary is now built with `CGO_ENABLED=0` (statically linked) so it also runs correctly when bind-mounted into an Alpine/musl-based runtime image, not just the Debian/Ubuntu-based images it previously required.
