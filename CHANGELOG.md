...
- **Per-repo agent toolchains via `mise`** — repos can now pin language versions (go, node, python, rust, ruby, java) that agent runs use instead of whatever the container image ships. A repo with no pins configured behaves byte-identically to before: the dispatcher only shells out to `mise`/`uv` when `repos.runtime_languages` is non-empty. When pins are set, the dispatcher runs `mise install` before invoking the provider and wraps the CLI command with `mise x`; a python pin gets a per-task-worktree virtualenv on `PATH` instead (so it doesn't shadow mise's own PATH handling). Toolchain prep failure escalates the task straight to `waiting_human` — it never silently falls back to an unprepared spawn. New `POST /api/v1/repos/{id}/runtime/detect` scans a repo's manifest files (`go.mod`, `.nvmrc`, `.python-version`, `rust-toolchain(.toml)`, `.ruby-version`, `.java-version`) and returns suggested pins; it only suggests, it never writes config. The Repos page has a new "Agent runtime" section (add/remove language + version rows, a "Detect from repo" button that pre-fills suggestions without auto-saving, and inline version validation) for configuring pins on create/edit. See `docs/runtime.md`.
- **Backend image base moved `node:26-alpine` → `node:26-bookworm-slim`** (breaking for anything built on top of the old image) — required for the toolchain feature above: `mise`'s prebuilt python/ruby/java runtimes need glibc, which alpine's musl libc doesn't provide. `mise` and `uv` are now baked into the image, and the compose files mount two new named volumes (`mise-data`, `uv-cache`) so installed toolchains and uv's package cache persist and are shared across repos. Expect a larger image than before (Debian's base is bigger than Alpine's); if you have a derivative image that `apk add`s on top of this one, it needs to switch to `apt-get`. `su-exec` was replaced by `gosu` (Debian's equivalent) in the entrypoint — no user-facing behavior change.
