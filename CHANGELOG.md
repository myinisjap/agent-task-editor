...
### Added
- **`runtime_languages` API (generated devcontainer)** — a repo can now set `runtime_languages` (`POST`/`PATCH /repos`) to a list of `{id, version}` pairs from a fixed language allowlist (go, node, python, rust, java, ruby); the backend generates the entire `devcontainer.json` server-side from this list — there is no free-form JSON field, by design (see `docs/runtime-containers.md`'s security rationale once T15 lands). An unknown id or an invalid version string is rejected with 400 naming the offending value, and omitting the field on a PATCH preserves the existing selection. New `GET /repos/{id}/devcontainer` reports which of the four runtime sources (`runtime_image` / a repo-committed `.devcontainer/devcontainer.json` / `runtime_languages` / none) currently governs a repo's agent runtime, plus the effective `devcontainer.json`, so a UI can warn when a committed file shadows the language picker.
- **Runtime environment picker on the Repos page** — the old plain-text `runtime_image` field is now a three-way "Runtime environment" choice on both the create and edit repo forms: None (run in the backend container, unchanged default), Image ref (the existing free-text image field), or Languages (a repeatable list of allowlisted-id + free-text-version rows via the new `RuntimeLanguagesEditor`). Still no raw devcontainer.json editor — that escape hatch is what made the earlier version of this UI a privilege-escalation path (see `AGENTS.md`); a repo needing more than the picker offers commits its own `.devcontainer/devcontainer.json`. Switching modes never discards what was typed into another mode within the same editing session. Editing a repo fetches `GET /repos/{id}/devcontainer` and shows an inline warning under Languages when the repo ships a committed devcontainer file that would shadow the selection. API 400s (unknown id / bad version) surface inline instead of a generic failure message.
- **Per-repo runtime containers (`runtime_image`)** — a repo can set an explicit Docker image ref (`repos.runtime_image`, a plain text field on the Repos page); when set, that repo's agent CLI runs inside its own long-lived container (`docker run -d ... <image> sleep infinity`, reused across runs) instead of the shared backend container, and every CLI invocation is `docker exec`'d into it. Leaving it unset (the default) is unchanged in-process behavior. The container gets the same mount contract the backend container already provides: the repo and `/tmp` bind-mounted at identical paths (required for git worktrees and the MCP sidecar handoff), the MCP sidecar binary, and provider credential directories (`~/.claude`, `~/.claude.json`, `~/.codex`, `~/.qwen`) mounted read-write so token rotation persists; plus `--security-opt=no-new-privileges`, `--cap-drop=ALL`, and a `--pids-limit`. A container is assumed to follow the devcontainer/VS Code convention (`HOME=/home/vscode`, a `vscode` user) — a plain upstream image like `golang:1.26` won't authenticate correctly; see `docs/runtime-containers.md`. `worktreesweep` reaps containers whose repo is gone or whose image has changed, skipping any repo with an in-flight run so it can't kill an active task. If the container can't be created or reached (Docker unavailable, image pull failure, etc.), the task escalates to `waiting_human` rather than silently falling back to in-process. **Requires the Docker socket**: `docker-cli` is now installed in the backend image, and `/var/run/docker.sock` is bind-mounted into the `backend` service in both compose files by default; `backend/entrypoint.sh` joins the backend's `node` user to the socket's owning group at container startup so this works regardless of what gid the host's Docker group happens to use. Mounting the socket gives the backend host-root-equivalent access — see `docs/runtime-containers.md`'s "Docker socket requirement" section before relying on this in a multi-tenant or untrusted-repo setup. The `mcp-server` sidecar binary is now built with `CGO_ENABLED=0` (statically linked) so it also runs correctly when bind-mounted into an Alpine/musl-based runtime image, not just the Debian/Ubuntu-based images it previously required.
