...
### Added
- **Per-repo runtime containers, via an explicit image or a language picker** — a repo can now run its agent CLI inside its own long-lived container instead of the shared backend container, resolved in this order: an explicit `repos.runtime_image` ref (`docker run -d ... <image> sleep infinity`, reused across runs and `docker exec`'d into per run); a repo-committed `.devcontainer/devcontainer.json`, passed to `devcontainer up` unmodified; or `repos.runtime_languages` — a picker, on the Repos page's new three-way "Runtime environment" choice (None / Image ref / Languages via `RuntimeLanguagesEditor`), of `{id, version}` pairs from a fixed allowlist (go, node, python, rust, java, ruby) that the backend generates into a full `devcontainer.json` server-side. Leaving all three unset (the default) is unchanged in-process behavior. There is deliberately no raw devcontainer.json editor anywhere in this feature — an earlier version accepted user-supplied JSON whose `runArgs`/`mounts` were merged with this codebase's hardening flags rather than replacing them, letting a `--privileged` entry survive and turn repo-edit access into host root; the generated path now sets every key from validated inputs only (unknown language id or malformed version is a 400), so no user string can reach a Docker flag or mount — a repo needing more commits its own devcontainer.json or uses `runtime_image` instead. See `docs/runtime-containers.md` for the full resolution order and security rationale. Every container variant gets the same mount contract (repo root and `/tmp` at identical host/container paths, the MCP sidecar binary, provider credential directories mounted read-write for token rotation) plus `--security-opt=no-new-privileges`, `--cap-drop=ALL`, and a `--pids-limit`; a container is assumed to follow the devcontainer/VS Code convention (`HOME=/home/vscode`, a `vscode` user), so a plain upstream image like `golang:1.26` won't authenticate correctly. New `GET /repos/{id}/devcontainer` reports which source currently governs a repo (plus the effective `devcontainer.json`), so the UI can warn when a committed file shadows the language picker. `worktreesweep` reaps containers whose repo is gone or whose image/generated-config hash has changed, skipping any repo with an in-flight run so it can't kill an active task. If a container can't be created or reached (Docker or `@devcontainers/cli` unavailable, image pull failure, etc.), the task escalates to `waiting_human` rather than silently falling back to in-process. **Requires the Docker socket**: `docker-cli` and a pinned `@devcontainers/cli` are installed in the backend image, and `/var/run/docker.sock` is bind-mounted into the `backend` service in both compose files by default; `backend/entrypoint.sh` joins the backend's `node` user to the socket's owning group at container startup so this works regardless of what gid the host's Docker group happens to use. Mounting the socket gives the backend host-root-equivalent access — see `docs/runtime-containers.md`'s "Docker socket requirement" section before relying on this in a multi-tenant or untrusted-repo setup. The `mcp-server` sidecar binary is now built with `CGO_ENABLED=0` (statically linked) so it also runs correctly when bind-mounted into an Alpine/musl-based runtime image, not just the Debian/Ubuntu-based images it previously required.
...
- **`POST /repos/{id}/detect-languages`** — suggests a repo's `runtime_languages` by scanning its manifest files (go.mod, package.json, etc.), falling back to a one-shot `claude` CLI call only when the scan finds nothing or finds evidence it can't confidently version (a range, or a manifest present with no version). The endpoint never persists anything — it returns `{suggestions, used_llm}`; only a subsequent `PATCH /repos/{id}` from the user writes `runtime_languages`. The LLM fallback's output is treated as untrusted model text: it's constrained by prompt to a JSON array of `{id, version}` restricted to the six allowlisted language ids, then run through the same `ParseRuntimeLanguages` validation as any other input — an unknown id or invalid version is dropped and logged, never sanitized or surfaced. A missing/misconfigured provider, timeout, or malformed LLM output degrades to the scan's own result rather than failing the request. A new "Detect from repo" button in the Repos page's Languages picker calls this endpoint and fills in rows with each suggestion's source (e.g. "from go.mod") and, when ambiguous or unversioned, a visible "needs confirmation" / "pick one" flag; a `used_llm` response shows "suggested by Claude — please confirm". Detecting an id already present replaces that row rather than duplicating it. Nothing is written until the user hits Save — the button never PATCHes, and a failed detect leaves the form as the user left it with an inline error.
