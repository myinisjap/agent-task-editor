...
- **Per-agent-config Claude permission mode (`permission_mode`)** — headless `claude` task runs previously passed no `--permission-mode`, falling back to the CLI's own default ("auto", a cloud safety classifier that can transiently deny harmless commands like `gofmt` with "Classifier unavailable"). Now configurable per claude-provider agent config: `""` (unset, today's behavior — byte-identical spawn), `default`, `auto`, `acceptEdits`, or `bypassPermissions`, validated at the API layer. `command_denylist` is still fully enforced even under `bypassPermissions` (verified against a live `claude` binary). Claude-only; other providers ignore the field. Interactive chat/terminal sessions are unaffected (a human is present to approve there). See `docs/agents.md#permission-mode` and `docs/providers/claude.md`.
...
- **Per-repo agent toolchains via `mise`** — repos can now pin language versions (go, node, python, rust, ruby, java) that agent runs use instead of whatever the container image ships. A repo with no pins configured behaves byte-identically to before: the dispatcher only shells out to `mise`/`uv` when `repos.runtime_languages` is non-empty. When pins are set, toolchain prep (`mise install`, and for a python pin, a per-task-worktree virtualenv via `uv venv`) runs in the run's own background goroutine — never on the dispatcher's sweep loop or an HTTP request — so a cold install can't freeze dispatch or get killed by a request timeout; the CLI command is then wrapped with `mise x` (a python pin gets `<worktree>/.venv/bin` on `PATH` instead, so it doesn't shadow mise's own PATH handling, plus a recorded-interpreter-version check that recreates the venv if the pin changed). A node pin gets its own adjustment: since claude/qwen's CLI binaries are themselves node scripts, `mise x`'s PATH prepend would otherwise make the CLI process run on the pinned node instead of the one it was built against — checked by inspecting the binary's actual shebang (never assumed), the spawn instead runs the CLI explicitly on the app's bundled node while node/npm/npx inside the agent's own Bash tool still resolve to the pin; codex/opencode's native binaries are unaffected either way, and a resolution failure fails the run closed rather than ever launching on the wrong interpreter. The venv is excluded from `git status`/`git add -A` so it can never leak into a task's branch. Toolchain prep failure escalates the task straight to `waiting_human` — it never silently falls back to an unprepared spawn. Interactive chat sessions apply the same pins (including the node fix) as task runs — python's venv lives outside the repo checkout for chat, since that worktree is a live, persistent checkout rather than a throwaway one. New `POST /api/v1/repos/{id}/runtime/detect` scans a repo's manifest files (`go.mod`, `.nvmrc`, `.python-version`, `rust-toolchain(.toml)`, `.ruby-version`, `.java-version`) — at the repo root and one subdirectory level deep, so monorepo layouts like `backend/go.mod` are found — and returns suggested pins; it only suggests, it never writes config. The Repos page has a new "Agent runtime" section (add/remove language + version rows, a "Detect from repo" button that pre-fills suggestions without auto-saving, and inline version validation, including a duplicate-language check mirrored server-side) for configuring pins on create/edit. See `docs/runtime.md`.
- **Backend image base moved `node:26-alpine` → `node:26-bookworm-slim`** (breaking for anything built on top of the old image) — required for the toolchain feature above: `mise`'s prebuilt python/ruby/java runtimes need glibc, which alpine's musl libc doesn't provide. `mise` and `uv` are now baked into the image (both selecting the correct amd64/arm64 release asset via `TARGETARCH`, so `arm64` builds — e.g. Apple Silicon under `dev.sh` — work the same as `amd64`), and the compose files mount two new named volumes (`mise-data`, `uv-cache`) so installed toolchains and uv's package cache persist and are shared across repos. Expect a larger image than before (Debian's base is bigger than Alpine's); if you have a derivative image that `apk add`s on top of this one, it needs to switch to `apt-get`. `su-exec` was replaced by `gosu` (Debian's equivalent) in the entrypoint — no user-facing behavior change. The entrypoint's ownership remap now skips the recursive `chown` on `mise-data`/`uv-cache` (and the other remapped state dirs) when ownership already matches the configured `PUID`/`PGID`, so a populated toolchain cache doesn't get walked on every container start. Both compose files' backend healthcheck switched from `wget` (a busybox built-in that doesn't exist on the Debian base) to `curl`. The entrypoint also now unsets the SSL/TLS override vars (`SSL_CERT_FILE`, `GIT_SSL_NO_VERIFY`, etc.) when they arrive as empty strings — compose defines them unconditionally, and a set-but-empty `SSL_CERT_FILE` made `mise` load zero CA certs and fail every toolchain download (an empty `GIT_SSL_NO_VERIFY` would likewise read as *true* to git). Proxy and CA-trust vars are additionally passed through to `mise`/`uv` during toolchain prep, so cold installs work behind corporate proxies.

### Changed
- **Provider Config `env` values are now write-only over the API — reads no longer return secret values.** `GET`/list `/api/v1/provider-configs`, and every response that embeds a resolved provider config (an agent config's `provider_config`, a chat session's `provider_config`), previously echoed `env` back verbatim, meaning any API client that could read an agent or provider config — including a future read-only token — got every configured API key in cleartext. Every read now masks each `env` value as the literal string `"***"`, keeping key names visible for the UI but never the value. Writes (`POST`/`PUT /api/v1/provider-configs`) resolve `env` against what's already stored: a value of `"***"` means "keep the existing value for that key" (this is what lets the Providers page round-trip a fetched, now-masked config without wiping secrets it never displayed), any other value (including `""`) is an explicit overwrite, and a key present in storage but omitted from the `env` object in the request is deleted; omitting `env` from the request body entirely still means "leave the whole thing unchanged," matching prior behavior. A malformed `env` (not a JSON object of string values) now 400s on both create and update, which is stricter than before. **Compat note:** a script that fetches a provider config and PUTs the entire body straight back now round-trips correctly (previously not exercised, so no regression there); a script that reads `env` expecting the real value back will now get `"***"` instead — it needs to hold onto the value it originally wrote rather than reading it back from the API. This is redaction in the API response only, not encryption: `env` is still stored in cleartext in SQLite, and `GET /api/v1/backup` (a raw DB snapshot) still contains real values by design — see `docs/agents.md#environment-variable-security`.
- **New tests assert `/api/v1/*` (including `GET /api/v1/backup`) is actually gated by `API_TOKEN` end to end** — `router_test.go` previously only exercised `/healthz` and `/metrics`; nothing proved the bearer-auth middleware group covering the rest of the API, most importantly the whole-database-dump backup endpoint, was wired up correctly. No behavior change, just closes a coverage gap.
...
### Fixed
- **A runaway agent subprocess (e.g. a test run spawning many worker processes) could exhaust the whole backend container's memory/CPU and starve the server itself** — `/readyz` timeouts, 500s and multi-second latencies on unrelated API requests, until someone noticed and killed it by hand. Two changes: every provider CLI subprocess (`claude`/`codex_cli`/`qwen_code`/`opencode`) now runs in its own process group (`backend/internal/agent/providers/cli.go`'s `newCLICommand`), so a run cancel or timeout kills the CLI's entire descendant tree instead of just the top-level process — previously, killing e.g. `claude` left whatever it had shelled out to (a test runner and its worker threads, `git`, `gh`, `mcp-server`) orphaned and still running, which is how a container ends up full of zombie processes from long-finished runs. Second, a new `memguard` background watchdog (`backend/internal/memguard`) checks the container's cgroup memory usage every 5s and, if it crosses 90% of the limit, cancels the single oldest in-flight run via the same `Pool.Cancel` path a human stop already uses — turning today's manual "notice it's stuck, kill it" into an automatic self-heal. No-ops outside a cgroup v2 container (e.g. local `dev.sh dev`).
- **Follow-up to the subprocess-tree fix above: capped JS test-runner parallelism so a single agent Bash-tool test run can't trigger it in the first place** — `os.cpus()` inside the backend container reports the host's full core count (no CPU cgroup limit is set), unrelated to the container's actual memory ceiling, so `npm run test`/`vitest run --coverage` with no pool config defaulted to spinning up as many worker threads as the host has cores — easily enough alone to exhaust a 4GB container (this is what both `memguard` cancellations above turned out to be). Every provider CLI subprocess's environment now includes `VITEST_MAX_WORKERS=4` by default (`backend/internal/agent/providers/cli.go`'s new `providerBaseEnv`); a repo or agent config that sets it explicitly still wins (`AgentConfig.Env` is appended after, and `os/exec` keeps the last occurrence of a duplicate key).
- **A run stopped by the `memguard` memory watchdog (see above) showed "Run cancelled by user" and left the task paused**, indistinguishable from a human deliberately stopping it and requiring someone to notice and manually resume the task. `Pool.Cancel` now takes an explicit reason: a human-triggered stop (via the API) still notes "cancelled by user" and pauses the task, while `memguard`'s new `CancelForResourceConstraint` records "Run stopped: container was low on memory..." and leaves the task unpaused so it's re-dispatched normally — a resource constraint isn't the task's fault.
- **Claude CLI bumped 2.1.223 → 2.1.238 to fix "Permission denied: Classifier unavailable" on every Bash command** — 2.1.223's auto-permission-mode safety classifier requests a model alias that stopped resolving server-side, so headless claude runs (which default to permission mode `auto`) had every classification-requiring Bash command denied; only statically-safe/read-only commands got through. Verified in-container: 2.1.223 denies 100%, 2.1.238 denies none with the same account, network, and environment.
...
- **Stored-XSS via attachment upload (#142)** — task attachments were sniffed as `image/*` on upload (good), but the *stored filename* kept the client-supplied extension (`filepath.Ext(fh.Filename)`), so a GIF/HTML polyglot uploaded as e.g. `evil.html` passed the image sniff, was written to disk as `.html`, and was later served by `GET /api/v1/uploads/{task_id}/{filename}` as `text/html` from the API origin — `http.ServeFile` there had no `X-Content-Type-Options` and inferred `Content-Type` purely from the extension. Two fixes: `saveUploadedAttachments` (`backend/internal/api/handlers/task_uploads.go`) now derives the stored extension solely from the *sniffed* MIME type (`image/png`→`.png`, `image/jpeg`→`.jpg`, `image/gif`→`.gif`, `image/webp`→`.webp`; any other sniffed type, including unsupported image types like `image/bmp`, is rejected with 400) — the client filename's extension, and the old `.bin` fallback for extension-less filenames, are no longer used. `UploadsHandler.ServeFile` (`backend/internal/api/handlers/uploads.go`) now always sets `X-Content-Type-Options: nosniff` and `Content-Disposition: inline; filename=""`, and sets `Content-Type` explicitly from a trusted extension→MIME map rather than `http.ServeFile`'s own extension-based inference. **Compat note:** attachments already on disk before this fix keep whatever extension they were originally stored with (no migration/rename is performed — the DB only stores the relative path) — `nosniff` plus the explicit `Content-Type` override for recognized extensions is what neutralizes those pre-existing files going forward; a legacy file under an extension outside the trusted map (e.g. a pre-fix `.html`) still relies on `nosniff` alone in the browser, since `Content-Type` inference for it is left to `http.ServeFile`.
