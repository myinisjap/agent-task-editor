...
### Added
- **Per-repo runtime containers (`runtime_image`, and now devcontainer.json)** — a repo's agent CLI runs can now execute inside their own container instead of the backend container, chosen per repo via a 4-way resolution order: an explicit `runtime_image` Docker ref (no build, wins outright) → a `.devcontainer/devcontainer.json` committed in the repo → `devcontainer_json` authored in the UI and stored on the repo → none (today's in-process behavior, unchanged default). The devcontainer sources are built with [`@devcontainers/cli`](https://github.com/devcontainers/cli) (`devcontainer up`), which handles installing the declared language features (Go, Node, Python, Rust, Java, Ruby, ...) and returns a container the agent CLI is then `docker exec`'d into, same as the explicit-image path. Before every devcontainer build, this codebase injects (into a copy of the config — a committed repo file itself is never modified) the same mount and hardening contract the explicit-image path already applies: same-path repo/`/tmp` mounts, the MCP sidecar binary, provider credential directories, and `--security-opt=no-new-privileges --cap-drop=ALL`; a repo's own `mounts`/`runArgs` entries are appended to, never replaced. The effective config is hashed and passed to `devcontainer up --id-label ate.dcjson=<hash>`, so an unchanged config reuses the existing container (~1s) and a changed one rebuilds (~2min cold for one feature); `worktreesweep` reaps containers whose label no longer matches. A `GET /repos/{id}/devcontainer` endpoint returns the fully-resolved config and which source won, so the UI can warn when a repo's committed file is overriding its saved settings. As with the explicit-image path, a build/start failure escalates the task to `waiting_human` rather than silently falling back to in-process. See `docs/runtime-containers.md`, including the current limitation that `@devcontainers/cli` still needs to be installed into `backend/Dockerfile` for the devcontainer sources to actually build. The `mcp-server` sidecar binary is now built with `CGO_ENABLED=0` (statically linked) so it also runs correctly when bind-mounted into an Alpine/musl-based runtime image, not just the Debian/Ubuntu-based images it previously required.
...
- **Runtime environment picker on the Repos page** — set a repo's runtime container from the UI: none (default, in-process), an explicit image ref, or a dev container built from a `devcontainer.json` you edit in-app (saved as the `devcontainer_json` source above). The dev container mode has a picker for common language features (Go, Node, Python, Rust, Java, Ruby — free-text version, since version lists rot) plus a collapsible raw JSON editor as an escape hatch for anything the picker doesn't model (mounts, `postCreateCommand`, etc.); picker edits only ever touch the language entries they own, so hand-written keys survive untouched. Warns inline when the repo ships its own committed `.devcontainer/devcontainer.json`, since that file always wins over the UI-authored config.
