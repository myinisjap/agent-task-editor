...
| [runtime-containers.md](runtime-containers.md) | Per-repo runtime containers: running a repo's agent CLI in its own container (an explicit `runtime_image`, a repo-committed devcontainer.json, or the generated-devcontainer language picker) instead of the backend container |
