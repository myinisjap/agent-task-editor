...
export type RuntimeLanguage = Schemas['RuntimeLanguage']
...
export const api = {
  ...
  repos: {
    ...
    // an omitted field from an explicit null (see ReposHandler.Update). The
    // same omit-preserves/present-replaces contract applies to
    // runtime_languages (an explicit [] clears it back to "not configured").
    update: (id: string, body: { name?: string; path?: string; remote_url?: string | null; workflow_id?: string | null; issue_sync_enabled?: boolean; issue_sync_label?: string; issue_writeback_enabled?: boolean; issue_writeback_label?: string; pr_review_auto_transition_enabled?: boolean; issue_sync_update_policy?: 'gate' | 'always' | 'never'; issue_sync_gone_action?: 'flag' | 'archive' | 'move'; issue_sync_gone_label?: string; issue_comment_sync_enabled?: boolean; max_concurrent_runs?: number | null; runtime_image?: string; runtime_languages?: RuntimeLanguage[] }) =>
