...
func (q *Queries) GetTaskBySource(ctx context.Context, arg GetTaskBySourceParams) (Task, error) {
	row := q.db.QueryRowContext(ctx, getTaskBySource, arg.Source, arg.SourceRef)
	var i Task
	err := row.Scan(
		&i.ID,
		&i.Title,
...
func (q *Queries) ListTasksBySourceRepo(ctx context.Context, arg ListTasksBySourceRepoParams) ([]Task, error) {
	rows, err := q.db.QueryContext(ctx, listTasksBySourceRepo, arg.Source, arg.RepoID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const setTaskPRMergeable = `-- name: SetTaskPRMergeable :one
UPDATE tasks
SET pr_mergeable = ?
WHERE id = ?
RETURNING id, title, description, type, label, repo_id, workflow_id, current_agent_run_id, agent_notes, active_agent_run_id, created_at, updated_at, branch, worktree_path, base_ref, attachments, git_state, paused, transient_retry_count, next_retry_at, source, source_ref, archived, pr_url, parent_task_id, created_by_run_id, merge_status, max_cost_usd, priority, writeback_in_progress_sent, writeback_pr_commented, writeback_closed, source_state, source_state_at, pr_mergeable, cost_warned, matched_rule_id
...
func (q *Queries) SetTaskPRMergeable(ctx context.Context, arg SetTaskPRMergeableParams) (Task, error) {
	row := q.db.QueryRowContext(ctx, setTaskPRMergeable, arg.PrMergeable, arg.ID)
	var i Task
	err := row.Scan(
		&i.ID,
		&i.Title,
...
const setTaskSourceState = `-- name: SetTaskSourceState :exec
UPDATE tasks SET source_state = ?, source_state_at = CURRENT_TIMESTAMP WHERE id = ?
`

type SetTaskSourceStateParams struct {
...
func (q *Queries) SetTaskSourceState(ctx context.Context, arg SetTaskSourceStateParams) error {
	_, err := q.db.ExecContext(ctx, setTaskSourceState, arg.SourceState, arg.ID)
	return err
}
...
const updateTaskFromSource = `-- name: UpdateTaskFromSource :one
UPDATE tasks
...
func (q *Queries) UpdateTaskFromSource(ctx context.Context, arg UpdateTaskFromSourceParams) (Task, error) {
	row := q.db.QueryRowContext(ctx, updateTaskFromSource,
		arg.Title,
		arg.Description,
		arg.Type,
		arg.ID,
