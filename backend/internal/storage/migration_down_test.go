...
//
// A later migration (019, agent_retry_policy) was added on top of 018 and
...
func TestMigration018DownStep(t *testing.T) {
	...
	// Migration 019 (agent_retry_policy) was added after 018 and doesn't
	// touch agent_runs, but Steps(-1) only rolls back the single most recent
	// migration, and Migrate(N) moves the schema to the state *after*
	// migration N has been applied (i.e. Migrate(18) only undoes 019, since
	// 18 is already applied at that point). To actually exercise 018's down
	// migration, roll back past it entirely to version 17.
	if err := m.Migrate(17); err != nil {
		t.Fatalf("down to version 17 (018 rollback): %v", err)
	}
