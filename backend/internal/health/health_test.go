...
func TestProviderChecks_OnlyEmittedWhenUsed(t *testing.T) {
	...
	// No providers configured → no qwen/opencode/anthropic/llm/codex rows.
	for _, id := range []string{"qwen_cli", "opencode_cli", "anthropic_api", "llm_api", "codex_cli"} {
		if hasID(checks, id) {
	...
	checks = Checks(Input{Providers: map[string]bool{"codex_cli": true}}, d)
	if !hasID(checks, "codex_cli") {
		t.Fatalf("expected codex check when provider in use")
	}
}
