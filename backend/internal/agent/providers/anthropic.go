...
func (r *AnthropicRunner) Run(ctx context.Context, input agent.RunInput, logCh chan<- agent.LogEntry) (agent.Result, error) {
	...
	for turn := 0; turn < maxTurns; turn++ {
		...
		if err != nil {
			...
			return agent.Result{Status: "failed"}, fmt.Errorf("anthropic messages turn %d: %w", turn, err)
		}
		if resp.Usage != nil {
		...
		for _, block := range resp.Content {
			...
			case "text":
				if block.Text != "" {
					logCh <- agent.LogEntry{Type: agent.LogStdout, Content: block.Text, At: time.Now()}
				}
			case "tool_use":
				logCh <- agent.LogEntry{Type: agent.LogToolCall, Content: fmt.Sprintf("%s(%s)", block.Name, block.Input), At: time.Now()}
...
func (r *AnthropicRunner) messagesComplete(ctx context.Context, model, system string, maxTokens int, messages []anthropicMessage) (anthropicResponse, error) {
	...
	if err != nil {
		...
		return anthropicResponse{}, &agent.ErrTransient{Cause: err}
	}
	defer func() { _ = resp.Body.Close() }()

	if resp.StatusCode == 429 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return anthropicResponse{}, &agent.ErrRateLimit{
	...
	if resp.StatusCode >= 500 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return anthropicResponse{}, &agent.ErrTransient{Cause: fmt.Errorf("http %d: %s", resp.StatusCode, body)}
	}
