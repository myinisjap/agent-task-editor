...
func TestAnthropicRunner_ExceedsConfiguredMaxTurns(t *testing.T) {
	...
	input := agent.RunInput{
		...
		AgentConfig: agent.AgentConfig{MaxTurns: 1},
		RepoPath:    t.TempDir(),
	}
...
func TestAnthropicRunner_CapturesUsageAndCost(t *testing.T) {
	...
	input := agent.RunInput{
		RunID:       "test-run",
		Task:        agent.Task{ID: "task-1", Title: "test task"},
		AgentConfig: agent.AgentConfig{MaxTurns: 5, Model: "claude-sonnet-4-5"},
		RepoPath:    t.TempDir(),
	}
...
func TestAnthropicRunner_SignalCompleteReadsOutcome(t *testing.T) {
	...
	input := agent.RunInput{
		RunID:       "test-run",
		Task:        agent.Task{ID: "task-1", Title: "test task"},
		AgentConfig: agent.AgentConfig{MaxTurns: 5},
		RepoPath:    t.TempDir(),
	}
...
func TestAnthropicRunner_GetTaskTransitions(t *testing.T) {
	...
	input := agent.RunInput{
		...
		AgentConfig: agent.AgentConfig{MaxTurns: 5},
		RepoPath:    t.TempDir(),
		Transitions: []agent.TransitionHint{{ToLabel: "review", Path: "success"}},
	}
