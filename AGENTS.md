...
- Agent `Bash`/`run_bash` tool calls execute inside the **backend** container against bind-mounted repos **only when the repo resolves to no runtime source** (empty `runtime_image` and no devcontainer.json — the default) — in that case any compiler/runtime an agent needs must be installed in `backend/Dockerfile`'s *final* image stage, not just available on the host. It currently ships Go 1.26 (copied from the Dockerfile's own builder stage for exact version parity with the server binary) and Node 26/npm (covers Vite/React/TS projects). To add another language for the no-runtime-source case, edit the final stage of `backend/Dockerfile` and rebuild — see `docs/getting-started.md#supported-languages--extending-the-toolchain`. `frontend/Dockerfile` is unrelated to this — it only builds/serves this app's own UI in production. A repo can instead resolve to a runtime container, in priority order: an explicit `runtime_image` (its agent CLI runs inside a container from that image via `docker exec`, `internal/agent/runtime.go` + `providers/cli.go`'s `spawn()`); a `.devcontainer/devcontainer.json` committed in the repo; or a `devcontainer_json` authored via the Repos page UI and stored on the repo — the latter two are built with `@devcontainers/cli` (`devcontainer up`, `internal/agent/devcontainer.go`) and cached by a hash of the effective config. Whichever source wins, the toolchain needs to be baked into that image/config, not the backend image — see `docs/runtime-containers.md`.
