...
- Agent `Bash`/`run_bash` tool calls execute inside the **backend** container against bind-mounted repos **only when the repo has no `runtime_image` set** (the default) — in that case any compiler/runtime an agent needs must be installed in `backend/Dockerfile`'s *final* image stage, not just available on the host. It currently ships Go 1.26 (copied from the Dockerfile's own builder stage for exact version parity with the server binary) and Node 26/npm (covers Vite/React/TS projects). To add another language for the no-runtime-image case, edit the final stage of `backend/Dockerfile` and rebuild — see `docs/getting-started.md#supported-languages--extending-the-toolchain`. `frontend/Dockerfile` is unrelated to this — it only builds/serves this app's own UI in production. A repo can instead set `repos.runtime_image` to an explicit image ref: its agent CLI then runs inside a long-lived container from that image via `docker exec` (`internal/agent/runtime.go` + `providers/cli.go`'s `spawn()`). This requires `docker-cli` in the backend image and `/var/run/docker.sock` bind-mounted in (both present in `docker-compose.yml`/`docker-compose.release.yml` by default; `entrypoint.sh` joins the `node` user to the socket's owning group at startup) — see `docs/runtime-containers.md`.
