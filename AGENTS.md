...
- Agent `Bash`/`run_bash` tool calls execute inside the **backend** container against bind-mounted repos **only when the repo's `runtime_image` is empty** (the default) — in that case any compiler/runtime an agent needs must be installed in `backend/Dockerfile`'s *final* image stage, not just available on the host. It currently ships Go 1.26 (copied from the Dockerfile's own builder stage for exact version parity with the server binary) and Node 26/npm (covers Vite/React/TS projects). To add another language for the no-`runtime_image` case, edit the final stage of `backend/Dockerfile` and rebuild — see `docs/getting-started.md#supported-languages--extending-the-toolchain`. `frontend/Dockerfile` is unrelated to this — it only builds/serves this app's own UI in production. When a repo sets `runtime_image`, its agent CLI runs inside a container from that image instead (`docker exec`, via `internal/agent/runtime.go` + `providers/cli.go`'s `spawn()`), and it needs the toolchain baked into `runtime_image`, not the backend image — see `docs/runtime-containers.md`.
