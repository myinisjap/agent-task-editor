...
│   ├── mcp-tools.md
│   ├── board-mcp.md
│   ├── runtime.md            # Per-repo agent toolchains via mise/uv
│   ├── backup.md
│   └── screenshots.md
...
- Agent `Bash`/`run_bash` tool calls execute inside the **backend** container against bind-mounted repos, so any compiler/runtime an agent needs must either be installed in `backend/Dockerfile`'s *final* image stage, not just available on the host, or pinned per-repo via the `mise`/`uv`-based runtime feature (see `docs/runtime.md`) — the image ships Go 1.26 (copied from the Dockerfile's own builder stage for exact version parity with the server binary) and Node 26/npm (covers Vite/React/TS projects) as its baseline; a repo can override/add to that per-task by pinning `go`/`node`/`python`/`rust`/`ruby`/`java` versions on its Repos page config, installed via `mise`/`uv` before the provider CLI runs (byte-identical spawn when no pins are set). To add a new language to the *image baseline* (not a per-repo pin), edit the final stage of `backend/Dockerfile` and rebuild — see `docs/getting-started.md#supported-languages--extending-the-toolchain`. `frontend/Dockerfile` is unrelated to this — it only builds/serves this app's own UI in production.
