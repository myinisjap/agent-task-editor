...
- **Chat tab** — a live, xterm-backed terminal in the browser running your provider's interactive CLI (`/chat`), one session per repo + provider config, each in its own git worktree; sessions survive disconnects so a refresh reattaches, and when `MCP_BOARD_PATH` is set (automatic in the Docker images) the session gets board tools for listing repos/workflows and creating tasks, so you can brainstorm a plan in chat and have it file the tickets — see [docs/board-mcp.md](docs/board-mcp.md)
...
- **First-run onboarding checklist** — a dismissible setup checklist on the Board for unconfigured instances, sequencing add a repo → configure a provider → create an agent config → create your first task, checking each off live and surfacing failing readiness checks inline
- **Configurable database cleanup** — automatic pruning of old agent logs (retention days + how often it runs) is DB-backed and editable at runtime from the Health page, no restart required — see [docs/backup.md#agent-log-retention](docs/backup.md#agent-log-retention)
...
Want the Codex/Qwen CLIs preinstalled instead of building them yourself (see below)? Pass `--all-cli` to run the `-all-cli` backend image variant published alongside each release:
...
Unlike `claude` (installed unconditionally), the Codex CLI (`codex_cli` provider) and Qwen CLI (`qwen_code` provider) are **not** installed in the default backend image — they're gated behind build args so the image doesn't grow for users who don't need them:

```bash
INSTALL_CODEX_CLI=true INSTALL_QWEN_CLI=true docker compose build
```
