...
```yaml
volumes:
  - /usr/local/bin/claude:/usr/local/bin/claude:ro   # CLI binary
  - ~/.claude:/root/.claude                           # auth credentials
```

Make sure the `claude` CLI is installed on your host and authenticated (`claude login`) before starting the stack.
